...
Steps in this code :
1. Set up of NXK Shot Learning Pipeline
    a. FSL Data Creation
    b. Embedding Generation
    c. Multimodal Siamese Network
    d. Training Set up
    e. Plotting functions for Analysis


2. Cyclical Learning rate Set up
    a. CyclicLR Function-callback for LR Range test
    b. Arousal and Valence Model setup for Training Hyperparameter Tests
...
from tensorflow.keras.layers import ConvLSTM2D, Conv1D, MaxPool1D,Conv2D, MaxPool2D, TimeDistributed,SimpleRNN, Conv2DTranspose,UpSampling2D, Bidirectional
...
from tensorflow.keras.callbacks import EarlyStopping, ReduceLROnPlateau, History, LearningRateScheduler
...
def create_dataset(df_spec, df_eda, df_bvp, df_ege, labels, shots, pid, f_path, spec_res, label_tag):

    #### Data Set Arrays
    t = labels[labels[label_tag].notna()]
    labels_n = t[t['pid'] == pid][label_tag]
    
    t = df_spec.loc[labels[label_tag].notna()]
    df_a = t[t['pid'] == pid]['MelFreq Spectrogram']

    t = df_eda.loc[labels[label_tag].notna()]
    df_e = t[t['pid'] == pid]

    t = df_bvp.loc[labels[label_tag].notna()]
    df_b = t[t['pid'] == pid]

    t = df_ege.loc[labels[label_tag].notna()]
    df_eg = t[t['pid'] == pid]    

    labels_n = labels_n.drop(columns =['pid'])
    df_a = df_a.drop(columns =['pid'])
    df_e = df_e.drop(columns =['pid'])
    df_b = df_b.drop(columns =['pid'])
    df_eg = df_eg.drop(columns =['pid'])
...
def reshape_pairs(ra, re, rb, ree,  labels):
      ...
      # a1 = a_1.reshape((a_1.shape[2], a_1.shape[3], a_1.shape[4]))
      # a2 = a_2.reshape((a_2.shape[2], a_2.shape[3], a_1.shape[4]))
...
def eda_embed(input_shape):
    ...
    # FC Block
    seq.add(Flatten())
    seq.add(Dense(64, activation=None, kernel_initializer="glorot_uniform", bias_initializer="zeros", bias_regularizer='l2', kernel_regularizer='l2'))
    seq.add(BatchNormalization())
...
def ege_embed(input_shape):
    ...
    # 1st FC Block
    seq.add(Dense(32, kernel_initializer="glorot_uniform", bias_initializer="zeros", bias_regularizer='l2', kernel_regularizer='l2'))
...
es2 = EarlyStopping(monitor='val_loss', patience=30, mode='auto', restore_best_weights=True)
es1 = EarlyStopping(monitor='val_binary_accuracy', patience=90, mode='max', restore_best_weights=True)
rop = ReduceLROnPlateau(monitor="val_loss", factor=0.85, patience=7, cooldown=5, verbose=1, min_lr = min_lr)
clr = CyclicLR(mode='triangular2',base_lr = base_lr, max_lr = max_lr, step_size = 5)

base_feat = 'kemocon/features/'
correct_pid = [15, 23, 30, 31]
feat_set =  ['_1s/']#,'_1s_h/', '_1s_k/', '_1s_l/', '_1s_o/']
shots_trial =  [5, 10, 15, 20, 25, 30]
base_model_p = 'kemocon/models_baseline_2/' 
...
for pid in correct_pid: 
    for f in feat_set:
        for shots in shots_trial:      
                ...
                labels = pd.read_hdf(features+'/self_labels.hdf5')

                #### Arousal Model
                base_dir = base_model_p + f + str(pid) + '/'
                os.makedirs(base_dir, exist_ok=True)
                model_name = 'EmobedA' + '_' + str(shots)              
                
                ## Create Dataset
                aud_tr, aud_ts, eda_tr, eda_ts, bvp_tr, bvp_ts, ege_tr, ege_ts, y_tr, y_ts = create_dataset(specm, eda, bvp, eg, labels, shots, pid, base_dir, 256, 'binary_arousal' )
                ...
                df1 = df1.append([[ pid, shots, np.count_nonzero(ltr),  ltr.shape[0] - np.count_nonzero(ltr), np.count_nonzero(lts),  ltr.shape[0] - np.count_nonzero(lts)]])               
                ...
                # history = model.fit([e1tr, b1tr, a1tr, eg1tr, e2tr, b2tr, a2tr, eg2tr], ltr, validation_data= ([e1ts, b1ts, a1ts, eg1ts, e2ts, b2ts, a2ts, eg2ts], lts) , 
                    # verbose = 2, workers = 2, epochs=epochs, batch_size = bs, shuffle = True, callbacks = [es1, rop])
                ...
                # plots1_a1(history=history.history, metric1="binary_accuracy",metric2="loss", label= "Performance of Arousal Prediction with " + str(shots) + " shots for Participant " + str(pid), base_dir = base_dir, model_name = model_name, pid = pid, i = shots,has_valid=True)
                
                # plots2_cm(lts, y_pred, pid = pid, plot_label=  str(shots) + "_Arousal", base_dir = base_dir)
...
# for pid in correct_pid: 
    # for f in feat_set:
        # for shots in shots_trial:      
                ...
                base_dir = base_model_p + f + str(pid) + '/'
                os.makedirs(base_dir, exist_ok=True)
                model_name = 'EmobedV' + '_' + str(shots)
                
                ## Create Dataset
                aud_tr, aud_ts, eda_tr, eda_ts, bvp_tr, bvp_ts, ege_tr, ege_ts, y_tr, y_ts = create_dataset(specm, eda, bvp, eg, labels, shots, pid, base_dir, 256, 'binary_valence' )
                ...
                df2 = df2.append([[ pid, shots, np.count_nonzero(ltr),  ltr.shape[0] - np.count_nonzero(ltr), np.count_nonzero(lts),  ltr.shape[0] - np.count_nonzero(lts)]])                
                ...
                # ## Plot of Contrastive Loss and Accuracy
                # plots1_a1(history=history.history, metric1="binary_accuracy",metric2="loss", label= "Performance of Valence Prediction with " + str(shots) + " shots for Participant " + str(pid),  base_dir = base_dir, model_name = model_name, pid = pid, i = shots, has_valid=True)
                
                # plots2_cm(lts, y_pred, pid = pid, plot_label=  str(shots)+ "_Valence", base_dir = base_dir)
...
df1.columns = ['pid', 'shots', 's 1', 's 0', 'q 1', 'q 0']
df2.columns = ['pid', 'shots', 's 1', 's 0', 'q 1', 'q 0']
