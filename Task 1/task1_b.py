...
2. Learning rate Range Test Set up
    a. LRFinder Function-callback for LR Range test
...
def plots3_p(base_dir, model_name, batch_size_variants, set_name, selected):
        ...
        for pid in selected:
            for f2 in f2_name:
                base_dir = 'kemocon/trial_hp_adam/' + str(pid) + '/'
                model_name = f2 + '_' + str(pid)
                for i in batch_size_variants:
                    ...
                    ax[0].plot(lr_new, df, label=   str(pid) + '_' + str(i)) 
                    ...
                    ax[1].plot(lr_new, df, label=  str(pid) + '_' + str(i)) 
                    ...
                    ax[2].plot(lr_new, df, label=  str(pid) + '_' + str(i)) 
        ...
        plt.savefig('kemocon/trial_hp_adam/m_Loss.png')        
        plt.close()
        ...
        plt.savefig('kemocon/trial_hp_adam/m_Accuracy.png')
        plt.close()
...
def plots3_s(base_dir, model_name, batch_size_variants, set_name):
        ...
        for i in batch_size_variants:
            ...
            ax[1].grid(True, which="major", ls="-")            
            ax[1].set_xlabel("Learning Rate", fontsize=17)
            ax[1].set_ylabel("Derivative of Query Loss", fontsize=17)
...
"""## 1s Set Learning Rate Range Test"""

f1s = 'kemocon/features/'
s_name = ['_1s/']   
f1_name = 'kemo_hp/' 

selected = [15, 23, 30, 31]
batch_size_variants = [8, 16, 32, 64]
set_name = '_1s'
shots = 20
epochs = 100
spec_res = 256
...
for pid in selected:
        ...
        #### Arousal Model
        base_dir = 'kemocon/' + f1_name + str(pid) + '/' 
        os.makedirs(base_dir, exist_ok=True)
        model_name = 'EA' + '_' + str(pid)
        ...
        # for i in batch_size_variants:
                ...
                # optimizer = optimizers.Adam()  ## SGD, RMSprop
        ...
        plots3_s(base_dir, model_name, batch_size_variants, set_name)                
